...
import { searchFlight } from "../../redux/action"
...
export default function SearchFlightBar() {
  ...
  useEffect(() => {
    if (value[0] !== null && value[1] !== null) {
      if (new Date(value[1].toString()).getTime() > new Date(value[0].toString()).getTime()) {
        const valTerm = value[0];
        const valTerm1 = value[1]
        setValue([valTerm1, valTerm])
  ...
  const handleSearch = () => {
    if (value[0] !== null && value[1] !== null && toDes !== "" && fromDes !== "" && valuePeople !== '') {
      const newDateStart = new Date(value[0].toString());
      const newDateEnd = new Date(value[1].toString());
      const valInput = {
        from: fromDes,
        to: toDes,
        timeStart: `${newDateStart.getDate()}/${newDateStart.getMonth() + 1}/${newDateStart.getFullYear()}`,
        timeEnd: `${newDateEnd.getDate()}/${newDateEnd.getMonth() + 1}/${newDateEnd.getFullYear()}`,
        people: Number(valuePeople),
      }
      dispatch(searchFlight(valInput));
      localStorage.setItem("searchFlight", JSON.stringify(valInput)); 
    }
    else {
  ...
  return (
    <RootStyle container md={9} xs={9}>
      ...
      <Grid container xs={9} md={2}>
        <Link href={value[0] !== null && value[1] !== null && toDes !== "" && fromDes !== "" && valuePeople !== ''?"/book-flight":"#"} underline="none"><SearchBtn variant="contained" onClick={handleSearch}> Search Flight</SearchBtn></Link>
      </Grid>
